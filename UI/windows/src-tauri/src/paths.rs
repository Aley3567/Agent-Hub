...
//!
//! Hub provider 位于 `~/.agent-hub/`；历史 JSON/日志仍位于 `~/.cc-switch/`（Windows 上就是 `%USERPROFILE%\.cc-switch`，
...
fn ensure_openable_in(base: &Path, provider_db: &Path, raw: &str) -> Result<PathBuf, String> {
    match ensure_inside(base, raw) {
        ...
        Err(boundary_error) => {
            ...
            let allowed = provider_db.canonicalize()
            ...
            if actual != allowed { return Err(boundary_error); }
            Ok(strip_verbatim(actual))
        }
    }
}
