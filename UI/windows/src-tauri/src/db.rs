...
pub fn claude_provider_rows() -> Result<Vec<ProviderRow>, String> {
    ...
    for required in ["id", "name"] {
        if !available.contains(required) {
            return Err(format!(
                "Hub provider 数据库的 providers 表缺少 {required} 列，渠道无法标识；请升级 CC Switch"
            ));
        }
    }
...
mod pricing_tests {
    ...
    fn missing_optional_pricing_does_not_require_a_provider_database() {
        ...
        let absent = std::env::temp_dir().join(format!("agent-hub-no-prices-{}-{}", std::process::id(),
    ...
    fn explicit_pricing_database_needs_no_provider_table_and_stays_read_only() {
        let path = std::env::temp_dir().join(format!("agent-hub-prices-{}-{}.db", std::process::id(),
            std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
