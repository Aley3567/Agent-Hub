...
export default function ManagementDialog({ mode, channel, onClose, onComplete }: {
  ...
  function summary(result: ProviderOutcome): string {
    return `新增 ${result.added}，更新 ${result.updated}，保留 ${result.skipped}${result.pendingCleanup ? `；待清理凭证 ${result.pendingCleanup}` : ''}`;
  }
  async function submit(): Promise<void> {
    ...
    try {
      ...
      if (mode === 'import') {
        ...
        message = `${summary(result.applied)}${result.storageCleanupPending ? '；本地存储清理未完成，可再次迁移重试' : ''}`;
      } else if (mode === 'delete' && channel) {
        const result = await api.removeProvider(channel.appType, channel.id);
        if (result.blockers.length) { setError(`渠道仍被引用，请先解除：\n${result.blockers.join('\n')}`); return; }
        message = `已删除 ${result.removed} 个渠道${result.pendingCleanup ? '；系统凭证待清理' : ''}`;
      } else {
        if (!id.trim() || !name.trim()) { setError('请填写稳定 ID 和名称。'); return; }
        if (mode === 'create' && (!endpoint.trim() || !model.trim() || !secret)) { setError('请填写 API 地址、模型和 API key。'); return; }
        if (secretAction === 'replace' && !secret) { setError('请填写新 API key。'); return; }
  ...
  const titles = { create: '新增渠道', edit: '编辑渠道', delete: '删除渠道', import: '导入渠道', migrate: '迁移凭证' };
  ...
  return <Dialog open title={titles[mode]} onClose={close} closeOnOverlay={!busy} footer={<>
    ...
    <Button variant={mode === 'delete' ? 'danger' : 'primary'} loading={busy} disabled={disabled} onClick={() => void submit()}>{mode === 'delete' ? '确认删除' : mode === 'migrate' ? '确认迁移' : mode === 'import' ? '确认导入' : '保存'}</Button>
  </>}>
    <div className={styles.form}>
      {error ? <p role="alert" className={styles.error}>{error}</p> : null}
      {mode === 'create' || mode === 'edit' ? <>
        <Field label="应用" htmlFor="provider-app"><Select id="provider-app" value={appType} disabled={mode === 'edit' || busy} options={[{ value: 'claude', label: 'Claude Code' }, { value: 'codex', label: 'Codex' }]} onChange={(e) => { const app = e.target.value as AppType; setAppType(app); setProtocol(app === 'codex' ? 'openai_responses' : 'anthropic'); }} /></Field>
        <Field label="稳定 ID" htmlFor="provider-id"><Input id="provider-id" value={id} disabled={mode === 'edit' || busy} onChange={(e) => setId(e.target.value)} /></Field>
        <Field label="名称" htmlFor="provider-name"><Input id="provider-name" value={name} disabled={busy} onChange={(e) => setName(e.target.value)} /></Field>
        <Field label="API 地址" htmlFor="provider-endpoint" hint={view ? `留空保留当前地址（${view.provider.endpoint ?? '未配置'}）` : undefined}><Input id="provider-endpoint" value={endpoint} disabled={busy} placeholder="https://api.example.com" onChange={(e) => setEndpoint(e.target.value)} /></Field>
        <Field label="协议" htmlFor="provider-protocol"><Select id="provider-protocol" value={protocol} disabled={busy || appType === 'codex'} options={['anthropic', 'openai_chat', 'openai_responses'].map((value) => ({ value, label: value }))} onChange={(e) => setProtocol(e.target.value)} /></Field>
        <Field label="默认模型" htmlFor="provider-model"><Input id="provider-model" value={model} disabled={busy} onChange={(e) => setModel(e.target.value)} /></Field>
        {mode === 'edit' ? <Field label="凭证操作" htmlFor="provider-secret-action"><Select id="provider-secret-action" value={secretAction} disabled={busy} options={[{ value: 'keep', label: '保留现有凭证' }, { value: 'replace', label: '替换 API key' }, { value: 'clear', label: '清除 API 凭证' }]} onChange={(e) => { setSecretAction(e.target.value); setSecret(''); }} /></Field> : null}
      ...
      {mode === 'import' ? <>
        <Field label="来源" htmlFor="provider-source"><Select id="provider-source" value={sourceKind} disabled={busy || preview !== null} options={[{ value: 'cc_switch', label: 'CC Switch' }, { value: 'claude', label: 'Claude 用户配置' }, { value: 'codex', label: 'Codex 用户配置' }, { value: 'json', label: 'version:1 JSON 文件' }]} onChange={(e) => setSourceKind(e.target.value as ProviderSource['kind'])} /></Field>
        <Field label="来源文件" htmlFor="provider-source-path" hint={sourceKind === 'json' ? '填写 JSON 文件完整路径' : '留空使用该应用的用户配置路径'}><Input id="provider-source-path" value={sourcePath} disabled={busy || preview !== null} onChange={(e) => setSourcePath(e.target.value)} /></Field>
        {sourceKind === 'codex' ? <Field label="Profile（可选）" htmlFor="provider-profile"><Input id="provider-profile" value={profile} disabled={busy || preview !== null} onChange={(e) => setProfile(e.target.value)} /></Field> : null}
        <Button disabled={busy || (sourceKind === 'json' && !sourcePath)} onClick={() => void scan()}>扫描并预览</Button>
        {preview ? <div className={styles.candidates}>
          {preview.blocked.map((reason, i) => <p key={`${reason}-${i}`}>不可导入：{reason}</p>)}
          {!preview.candidates.length ? <p>此来源没有可导入渠道。</p> : null}
          {preview.candidates.map((candidate) => <label className={styles.candidate} key={candidate.candidateId}>
            ...
            <span>{candidate.name} · {candidate.appType}<small>{candidate.endpoint ?? '未配置地址'} · {candidate.blockedReason ?? (candidate.conflict ? '已有同 ID' : '新增')}</small></span>
          </label>)}
          <Switch label="覆盖所选已有渠道" checked={replace} disabled={busy} onChange={(value) => { setCounts(null); setReplace(value); }} />
          {counts ? <p>将新增 {counts.added}，更新 {counts.updated}，保留 {counts.skipped}</p> : null}
        </div> : null}
      </> : null}
      {mode === 'delete' ? <p>删除 {channel?.name}（{channel?.appType}）？已有会话继续运行；仍被 Hub 或账号池引用时将阻止删除。</p> : null}
      {mode === 'migrate' ? <><p>将 Hub 库的旧凭证移入系统凭证库。启动仍可能使用受限临时认证文件，历史备份保留原样。</p><p>{status ? `待迁移 ${status.legacy}，已引用 ${status.referenced}，待清理 ${status.pendingCleanup}` : '正在检查凭证状态…'}</p></> : null}
    </div>
  </Dialog>;
}
