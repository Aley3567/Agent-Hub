...
渠道页以选择和启动为主：渠道名称（弹性列，至少 150px）、模型（240px）、最近记录
（160px）、操作（240px）。继续使用现有 `--table-col-*` token，总最小宽度 790px。
渠道名允许换行，当前标记和中性的配置状态放在名称下；协议、上下文窗口与语义兼容性
保留在展开详情。操作列明确显示「启动会话」与「更多」，隐藏操作放在详情中。
最近记录只按稳定 provider ID 与 app 关联，不用同名/别名猜测，也不把用量记录称为成功。
打开渠道页不触发主动测活。
...
Agent 对话模式。会话有两类来源：**历史**（只读回放的 `~/.claude/projects` transcript）
与**本地**（本应用新建、经 hub 真发送、落 `agent-hub-chat.json`）。结构是工作台三件套：
左侧会话列表 + 右侧消息流 + 底部 composer，顶部再加一行项目选择器。
本视图**满宽**（同表格类视图，不走 1120px 居中），且自身接管滚动——会话列表与消息流是
两个独立滚动容器，内容区外壳不滚。这是 §3「唯一滚动容器」的唯一视图级例外。

**项目选择器（顶部单行）**

历史会话按项目目录切分。选择器列出 `~/.claude/projects` 下**真有 transcript** 的项目，
label 用项目的真实路径（由 Rust 从 transcript 的 `cwd` 字段读出——目录名到路径不可逆，
不能反解）。未选中任何项目时不显示历史会话。

**只读 / 可写的区分（多重编码，不靠颜色）**

历史会话不可发送。这条**必须同时用三种方式表达**：会话列表项上的锁形图标、文字「历史」
标识、以及 composer 的 placeholder 说明只读原因。只禁用输入框而不说明原因，等于让用户
对着一个没反应的控件猜。

**会话列表（左栏，固定宽 `--chat-list-w` 240px）**

- 与消息流之间 1px `--border-subtle`（兄弟分隔线，同 §2.2.1 外壳条带口径）。
- 每项两行：标题一行截断；副行「来源 · 渠道名或 hub 名 · 相对时间」mono `--fs-12`
  `--text-tertiary`。
...
- 用户消息：右对齐窄块，`--bg-elevated` 底 + `--border-default` 描边 + `--radius-md`，
  最大宽为消息流宽的 70%。这是全站仅有的「气泡」，且**不自造彩色底**。
- 助手消息：无气泡，正文流左对齐全宽（ChatGPT / Codex 的克制做法），正文 `--fs-14`。
- system 提示（含截断说明）：`--bg-inset` 小条（`--radius-sm`）+ `--fs-12` `--text-tertiary`。
...
**失败不伪装成功**：流失败时只在 toast 里如实报错，**消息流里不插入任何助手气泡**——
一个半截的回复气泡看起来就像回复成功了。被上游截断（干净结束但没有终止帧）是另一回事：
正文确实收到了，照常渲染并由一条 system 小条说明截断，播报文案也要说「已保存收到的部分」，
不能说「完成」。三条流式事件的语义见 `CONTRACT.md` §3.2。

**hub 未运行时的空态**

视图保留，显示「claude-hub 未在运行」+ 怎么起 + 重试按钮。按 §4.5 的空态规范给下一步动作，
不写「暂无对话」。**绝不退回演示数据**——chat 根本不提供 mock（`CONTRACT.md` §4）。
...
用户确认以完整可读优先：通用表格正文 12px、表头 11px，数值右对齐且不换行；自动列宽中的名称不再使用 max-width:0，长标识允许换行。保留各平台行高和滚动容器。用量明细固定列预算、20行分页，详情弹窗承载完整元数据；不做待回填展示。此规则替代早期强制单行和中截断的文字约定。
