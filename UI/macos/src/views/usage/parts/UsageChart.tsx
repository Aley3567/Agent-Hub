...
const LABELS:Record<string,string>={claude:'Claude Code',codex:'Codex',get unknown() { return t("未记录"); },get in() { return t("普通输入"); },get out() { return t("输出"); },get cr() { return t("缓存读取"); },get cw() { return t("缓存写入"); }};
...
export function UsageChart({usage,names={}}:{usage:UsageSummary;names?:Record<string,string>}){
 ...
 return <section className={styles.chart} aria-label={t("用量趋势")}>
  <div className={styles.heading}><div><strong>{t("用量趋势")}</strong><p>{t("点击图例筛选，点击柱子查看时段明细")}</p></div><div className={styles.controls}>
   <div role="group" aria-label={t("图表指标")}>{([['tokens',t("Token 用量")],['cost',t("估算费用")]] as const).map(([v,l])=><button key={v} aria-pressed={metric===v} onClick={()=>setMetric(v)}>{l}</button>)}</div>
   <div role="group" aria-label={t("堆叠维度")}>{([['harness',t("按 Harness")],['provider',t("按 Provider")],['components',t("按构成")]] as const).map(([v,l])=><button key={v} aria-pressed={group===v} onClick={()=>{setGroup(v);setHidden(new Set());setSelected(null);}}>{l}</button>)}</div>
  </div></div>
  <div className={styles.scroll}><svg viewBox={`0 0 ${w} 238`} style={{minWidth:w>1000?w:undefined}} role="img" aria-label={t("{0}堆叠柱状图", [metric==='tokens'?'Token':t("估算费用")])}>
   ...
   {points.map((p,i)=>{let y=bottom;const active=p.slices.filter(s=>!hidden.has(s.key)),unknown=metric==='cost'&&active.some(s=>s.value===null);const description=t("{0} · {1} 条用量记录\n", [formatTime(p.t), p.turns])+active.map(s=>`${label(s.key)}：${s.value===null?t("未知"):fmt(s.value)}`).join('\n');return <g key={p.t} tabIndex={0} role="button" aria-label={description} onMouseEnter={()=>setSelected(i)} onFocus={()=>setSelected(i)} onClick={()=>setSelected(i)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')setSelected(i);}} className={styles.bar}>
    ...
    {(i%Math.max(1,Math.ceil(points.length/9))===0||i===points.length-1)&&<text x={left+(i+.5)*step} y={228} textAnchor="middle" className={styles.axis}>{new Date(p.t*1000).toLocaleString(useLocale.getState().language,usage.granularity==='hour'?{month:'numeric',day:'numeric',hour:'2-digit'}:{month:'numeric',day:'numeric'})}</text>}
   </g>;})}
  </svg></div>
  ...
  <div className={styles.detail} aria-live="polite">{detail?<><strong>{formatTime(detail.t)}</strong><span>{detail.turns}{t(" 条用量记录")}</span>{detail.slices.filter(s=>!hidden.has(s.key)).map(s=><span key={s.key}>{label(s.key)} <b>{s.value===null?t("未知"):fmt(s.value)}</b></span>)}</>:<span>{t("选择时段查看明细")}</span>}</div>
  {metric==='cost'&&<p className={styles.note}>{t("“?” 表示该时段有未定价模型或缺失用量字段；已知部分不代表完整费用。")}</p>}
  {!!usage.incompleteTurns&&<p className={styles.note}>{usage.incompleteTurns}{t(" 条记录缺少部分 Token 字段；仅累计已报告用量；缓存率使用输入字段完整的记录，完整费用仍为未知。")}</p>}
 </section>;
}
