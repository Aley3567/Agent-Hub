...
export const FAILURE_KIND_LABEL: Record<FailureKind, string> = {
  get server() { return t("上游服务端错误"); },
  get client() { return t("请求被拒"); },
  get aborted() { return t("连接中断"); },
  get unknown() { return t("状态不明"); },
};
...
const PHASE_LABEL: Record<string, string> = {
  get connect() { return t("建立连接"); },
  get request() { return t("发送请求"); },
  get response() { return t("读取响应"); },
  get stream() { return t("流式传输"); },
};

/** 连接断在哪个阶段，决定「已经拿到的东西还算不算数」，这是最值钱的一句 */
const PHASE_STORY: Record<string, string> = {
  get connect() { return t("还没连上上游就断了，这一轮什么都没发出去。"); },
  get request() { return t("请求已经发出去，上游没有把回应说完。"); },
  get response() { return t("上游回了响应头，读响应体时断了。"); },
  get stream() { return t("流已经开始，中途被切断——之前收到的内容是真的，但这一轮没有正常收尾。"); },
};
...
const STATUS_GLOSS: Record<number, StatusGloss> = {
  400: {
    get note() { return t("上游认为请求本身不合法"); },
    get hint() { return t("看下面的原文：多半是某个字段的形状上游不接受。协议桥默认放行方言，所以这类拒绝通常来自上游而不是转换。"); },
  },
  401: {
    get note() { return t("凭证被拒"); },
    get hint() { return t("桌面端只显示凭证「已配置 / 未配置」，具体值要去 CC Switch 里核对，本工具不读也不显示 key。"); },
  },
  403: {
    get note() { return t("凭证有效但没有权限"); },
    get hint() { return t("确认这个 key 是否开通了该模型；换个模型或换渠道试一轮就能区分是账号权限还是模型权限。"); },
  },
  404: {
    get note() { return t("上游没有这个路径"); },
    get hint() { return t("核对渠道端点：BASE_URL 末尾是否多写或漏写了 /v1 这类路径段。"); },
  },
  405: {
    get note() { return t("上游不接受这个请求方法"); },
    get hint() { return t("端点对得上却报 405 时，多半是上游网关的路由问题而不是本机配置；隔一段时间用同一渠道再跑一轮，看是不是一直如此。"); },
  },
  408: {
    get note() { return t("上游等超时了"); },
    get hint() { return t("缩短单轮输入或降低并发再试；连续超时就换渠道，本机改不动上游的等待时间。"); },
  },
  413: {
    get note() { return t("请求体超过上游上限"); },
    get hint() { return t("这一轮的上下文太大。裁历史、或换一个上下文窗口更大的渠道。"); },
  },
  422: {
    get note() { return t("上游校验不通过"); },
    get hint() { return t("看原文里指的是哪个字段；工具参数 schema 被规整过的场景会记 HUB_DEGRADE_SCHEMA_NORMALIZED，可以到降级区对一下时间。"); },
  },
  429: {
    get note() { return t("被限流"); },
    get hint() { return t("降低并发或等一会儿再试。配了账号池的渠道会自动轮换，可以到账号池视图看是不是所有成员都在冷却。"); },
  },
  500: { get note() { return t("上游内部错误"); }, get hint() { return t("上游服务端的问题，本机配置改不动它；换渠道或换槽位继续干活，回头再看是不是一直如此。"); } },
  502: { get note() { return t("上游网关拿不到后端"); }, get hint() { return t("同 500：这是上游链路的问题。连续出现就先换渠道。"); } },
  503: { get note() { return t("上游暂时不可用"); }, get hint() { return t("通常是过载或维护，隔几分钟再试；同一时间点多个渠道都 503 才需要怀疑本机出网。"); } },
  504: { get note() { return t("上游网关超时"); }, get hint() { return t("上游后端没在网关的时限内回话。缩短单轮输入或换渠道。"); } },
};

const CLASS_HINT: Record<FailureKind, string> = {
  get server() { return t("上游服务端的问题，本机配置改不动它；换渠道或换槽位继续干活。"); },
  get client() { return t("上游明确回绝了这次请求，状态码与错误体都是原样转下来的，按上游文档解读下面的原文。"); },
  get aborted() { return t("没有 HTTP 状态可查，只能按时间点去 hub 日志里对齐同一轮请求。"); },
  get unknown() { return t("这条记录既没有状态码也没有异常类型，能给出的线索只有时间、渠道和阶段。"); },
};
...
export function describeFailure(row: ErrorRow): FailureNarrative {
  ...
  const phaseLabel = phaseRaw === null ? t("未记录") : (PHASE_LABEL[phaseRaw] ?? phaseRaw);
  ...
  if (row.status !== null) {
    ...
    const head = gloss === undefined ? t("上游返回了 HTTP {0}", [row.status]) : t("上游返回 HTTP {0}：{1}", [row.status, gloss.note]);
    ...
    const tail =
      message !== null
        ? t("下面是错误流水里记下的原文（已脱敏）。")
        : code !== ''
          ? t("错误流水里没有 message，只记下了错误 code {0}。", [code])
          : t("错误流水里连 message 都没有记下。");
    explain = t("在「{0}」阶段，{1}。{2}", [phaseLabel, head, tail]);
  } else if (exc !== '') {
    statusText = t("连接中断");
    statusNote = t("上游未给出错误体");
    const story = PHASE_STORY[phaseRaw ?? ''] ?? t("连接在中途断开了。");
    explain = t("{0}没有 HTTP 状态，journal 里只留下异常类型 {1}。", [story, exc]);
  } else {
    statusText = t("状态未记录");
    statusNote = null;
    explain = t("在「{0}」阶段记下了一次失败，但 journal 既没有状态码也没有异常类型。", [phaseLabel]);
  }
