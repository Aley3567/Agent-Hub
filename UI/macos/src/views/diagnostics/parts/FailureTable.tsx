...
export function FailureTable({ items, expanded, onToggle, onInspectDegrade }: FailureTableProps) {
  return (
    <Table stickyHeader minWidth={1080} aria-label={t("失败记录")}>
      <thead>
        <tr>
          <Th aria-label={t("展开详情")} />
          <Th>{t("时间")}</Th>
          <Th>{t("阶段")}</Th>
          <Th>{t("渠道")}</Th>
          <Th>{t("模型")}</Th>
              ...
              <tr data-row="">
                <Td>
                  <button
                    ...
                    aria-expanded={open}
                    aria-label={open ? t("收起这条失败的详情") : t("展开这条失败的详情")}
                ...
                <Td mono truncate title={row.channel ?? t("流水未记录渠道")}>
                  {row.channel ?? MISSING}
                </Td>
                <Td mono truncate title={row.model ?? t("流水未记录模型")}>
                  {row.model ?? MISSING}
                </Td>
                <Td>
                  <Badge tone={formatTone(row.format)}>{row.format ?? t("未记录")}</Badge>
                </Td>
                <Td>
                  <span className={styles.stack}>
                ...
                <Td mono truncate title={row.exc ?? t("没有记录异常类型")}>
                  {row.exc ?? MISSING}
                </Td>
                  ...
                  <td className={styles.detailCell} colSpan={COLUMN_COUNT}>
                    <div className={styles.detail}>
                      <dl className={styles.facts}>
                        <dt className={styles.term}>{t("这条失败在说什么")}</dt>
                        <dd className={styles.text}>{narrative.explain}</dd>
                        <dt className={styles.term}>{t("下一步")}</dt>
                        <dd className={styles.text}>{narrative.hint}</dd>
                      </dl>
                      {narrative.message === null ? (
                        <p className={styles.text}>{t("错误流水没有记下 message，能看到的线索只有上面这几列。")}</p>
                      ) : (
                        <CodeBlock code={narrative.message} label={t("错误流水原文（已脱敏）")} wrap />
                      )}
                      <div className={styles.tags}>
                        {row.code === null || row.code === '' ? null : (
                          <Badge tone="neutral" title={t("上游给出的错误 code")}>{`code ${row.code}`}</Badge>
                        )}
                        {row.route === null || row.route === '' ? null : (
                          <Badge tone="neutral" title={t("这次请求命中的路由")}>{`route ${row.route}`}</Badge>
                        )}
                        {row.deg.map((code) => (
