...
test_first_install_is_safe() {
  ...
  command cmp -s "$REPO_ROOT/claude1_account_pool.py" \
    "$home/install root/scripts/claude1_account_pool.py" ||
    fail "account pool module was not installed"
  ...
  command cmp -s "$REPO_ROOT/claude1_routing.py" \
    "$home/install root/scripts/claude1_routing.py" ||
    fail "routing module was not installed"
