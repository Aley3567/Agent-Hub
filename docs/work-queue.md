...
**目的**：以 Model / Protocol Gateway 为主线，让源码、README、架构图与面试表达一致。
不扩建 Agent Loop、Tool Execution、Memory 或 Sandbox，不一次重写所有运行模块。

**依据**：2026-09-08 用户要求；[当前调用链与证据](request-flow.md)。每次重要修改都说明
原问题、改变的控制流、是否改变行为、对应测试与运行态不确定性。

### Phase 1 · 固定入口和真实调用链

**状态**：✅ 2026-09-08。两名 Astra 独立核对，调用链与 README 已落盘；文档索引 6 项通过，
修改前 Python 全量 959 项通过。本阶段没有运行时代码变化。

**范围**：新增调用链导读、纠正 README 中入口/凭证/重试描述、更新文档索引。
标明 root Python 是主运行时，src 管理面、Go 实验和桌面演示不混入主链能力。
**验收**：按真实函数核对主线和三种协议分支；文档索引通过；运行行为不变。
...
**验收**：修复前按实际清单失败，修复后 help/version 可执行；不读取真实 DB、不发上游请求；
Python 全量与相关交付检查通过。只修本地包，不发布 npm。

### Phase 2b · 模型路由责任独立

**状态**：✅ 2026-09-08。新增 `claude1_routing.py`，HTTP handler 显式传入已取得的 provider
快照；移除 route 内隐式读库，内部第三参改为必填。Astra 对照 1,248 组输入的返回与异常一致；
新增 6 项匹配/快照测试，全量 967 项通过；npm 产物 2 项、安装 6 项通过。HTTP 行为保持不变。
同步 npm/install 清单与备份规则，未部署至当前用户运行目录，真实上游运行态未验证。

**范围**：先核对 route() 的全部调用方和测试依赖，再提取 selector → target 的纯逻辑；
provider 快照在 IO 边界读取，显式传入。保持匹配优先级、错误和 model 重写行为。
**验收**：裸槽位、显式渠道、歧义、1M、官方模型别名、未知模型和 route 组测试保持通过；
纯路由不查 DB、不发网络；同步安装清单，现有命令继续可运行。
若提取必须引入延迟回调或兼容套层，缩小迁移面，不用复杂机制换文件变短。

### Phase 3 · 按责任整理核心模块

**顺序**：先 launcher 的 TUI/启动责任，再 Hub 配置和 provider snapshot owner，最后协议请求/
响应/流模块。每次只搬一个责任；已有 `UpstreamExecutor`、`AccountPool`、`UsageReceipt`、
显式 adapter 字典和工具因果保护保留。原生/转换流有真实语义差异，不为统一接口强行合并。
**验收**：每次改动列清调用前后路径，协议 golden/流终态与相关集成测试通过；搬到 src 前
先证明 Python/npm/install 三种依赖和入口闭包。兼容代码先找调用与导出合同再决定删除。

### Phase 4 · 只补已证实的运行与工程缺口

- 下游失活误重放：沿用 **S20** 的复现与验收合同，不另建相同修复。
- Standalone 协议：`launch_standalone_session` 尚未消费 adapter。先选择并明确支持合同，
  再接现有桥或限制可启动协议；测试不能只证明 profile 能存枚举，必须验证最终请求格式。
- Standalone 凭证：`MacOSKeychainStore.set_secret` 目前将 secret 放进 subprocess argv。
  改用不暴露 argv 的系统接口前先核实可用机制；验证命令参数与错误信息不含凭证，再在 macOS
  验证真实凭证库操作。此前不得宣称已发生泄露或已经修复。
- transport 与原生重放：明确“没有响应/下游未交付”不证明上游没执行，测尝试次数与取消行为；
  不先加指数退避、熔断器或更多重试层。

**已有能力**：JSONL 错误/用量日志、超时、账号/transport 切换、healthz/readyz 与隔离集成测试。
不重复造同名能力。每个新增测试都绑定真实缺口。

### Phase 5 · 展示与操作收口

**范围**：README 依次说明问题、定位、图、请求流程、模块、支持协议与模型配置、Quick Start、
Demo、设计决策、已知限制；现有历史设计的状态描述与现行代码区分。
用本地 fixture 展示一次文本、一轮工具调用、一次错误/终态失败，再按需要验真实渠道。
Go 实验终态规则与主 Python 不一致；先写清实验身份，产品身份未决定前不合并、不整库删除。
**验收**：新用户按文档能启动，能从 handler 找到真实 HTTP 发送点，能解释 route 与四层 retry；
截图/录屏来自实际所宣称的路径，UI 演示回复不能充当真实上游证据。

**全阶段提交边界**：每个已验证行为独立本地 commit，不混合既有 `.agents/` 工作；不推送或发布。
